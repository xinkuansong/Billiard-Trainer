...
- [x] 日历翻月正常，有记录的日期正确高亮（使用 P4 创建的测试数据）— TP-P6 V-01~V-07, Flow 1~2 全通过
- [x] 训练详情完整展示（所有 Drill、分组数据、心得）— TP-P6 V-08~V-12, Flow 1 全通过
- [ ] 统计图在有数据时正常绘制，切换周/月/年后图表更新 — 统计为 Pro 专属（符合规格），免费用户 paywall 正确生效，Pro 体验待 TestFlight 补测
- [ ] 空数据状态（首次使用）展示引导，不显示空白图表 — 同上，需 Pro 身份测试
- [ ] 免费用户：60 天前的记录有遮罩，60 天内正常访问 — 代码逻辑审查通过（T-P6-06 DoD ✅），e2e 需 >60 天数据补测
- [x] Dark Mode：图表颜色适配正常 — TP-P6 D-01~D-04 通过（日历/详情部分）

**QA-P6 结论**：✅ 附条件通过（2026-04-12）
