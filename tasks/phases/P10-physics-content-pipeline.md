...
### ADR-P10-10 — v63软袋收集与可见收尾（2026-09-13，实施中）
- **背景/依据**：用户指出袋底固定终态求解投入过长，要求简单方案；v63§5.7原已允许不可逆结果/必要可见运动完成后的停止。
- **选择**：台面/袋角/硬质袋壁仍由局部物理决定进入与返回。袋网使用吸收式收集边界，最低硬质几何与袋网截面来自当前资产；球完整通过硬质区域、向下且与活动球脱离时确认候选，统一调度提交。确认后短可见运动独立于袋底长期堆积求解，所有消费者共用记录。
- **取舍**：不模拟软袋网绳及长期堆积弹性。候选不等于已提交规则事件，靠近其他活动球时延后；不能恢复旧XZ捕获后伪造下落作为最终交付。原完整袋底探索失败不改成通过。
- **证据/待验**：collection-boundary-r1三测与normal-entry-collection-r1一测通过，实测六袋截面图已核对；真实临界/连续多球、调度生产接入、可见收尾、回放导出/真机仍待验。


### ADR-P10-11 — 默认统一局部进袋入口（2026-09-13，全面验收中）
- 场景：v63混合求解/软袋收集与真实消费者已接，继续保留默认旧圈会使用户页面无法使用新链路。
- 决策：所有有实际调用的预测/开球/反射入口使用不可变SimulationModel.appDefault局部模式；planarReference显式保留用于旧基准对照。普通台面仍由原平面演进，局部区域负责高度/捕获，收集后固定终态短收尾。
- 边界：覆盖ADR-P10-09默认“球心入孔圈即落袋”机制，保留画面/规则同源原则；不变更独立几何理论评分。台呢竖向恢复.3是候选，不宣称标定完成。
- 证据：DR-234两项真实局部反解、默认中袋捕获、标准完整3D页面流程通过。W07全面差异回归/手机成本与W08导出一致性仍未验完；未发布。

### ADR-P10-12 — 空间调度增加腾空域（2026-09-14，实施中）
- 场景：主入口远离袋口的初始腾空球被当作平面球；空间支撑与联合碰撞已有实现。
- 选项：另建全三维引擎；以最近袋ID代替空中域；在现有空间所有权中增加显式airborne域。
- 决策：增加airborne域，复用全桌静态候选、连续球球接触、共同时间与提交机制。普通球继续平面演进，恢复可靠支撑后交回；实际进袋边界独立决定捕获。
- 原因：避免重复引擎与虚假袋口语义，保持预测、规则、回放同一运动记录。旧袋口域仍保留退出区域条件，空中域只要求支撑和接触组分离。
- 验证：DR-290重力反例修复、越球/低位碰撞及四组抬杆落台停稳通过，旧所有权/续算/捕获四项通过。普通物理类仍原10失败方法，空中库边/出界/正式输入/设备性能未验收。

### ADR-P10-13 — 袋口内衬由刚性碰撞面改为耗能体（2026-09-14，本地验收）
- 场景：v63 空间模型把 USDZ `Leather` 面当刚体墙（借用旧喉壁 e=0.45、库边 μ=0.2），球在 4 m/s 级进袋时被竖直半圆杯壁切向导回台面；W07 原 10 个常规进袋失败多数属此类。实物（乔氏台角袋照片，`output/3d-v63/W07/liner-audit/joy-corner-pocket-real-20260914.jpg`）皮革是挂在台框外、顶沿不高于台呢、松垂的软革围裙，只接球吸能；能顶回球的只有库鼻。
- 选项：①继续调皮革 e/μ；②重塑内衬几何为导球斜面（W07 契约文件已暂停的候选）；③把内衬接触定义为耗能体：法向恢复 0，逼近接触后球心切向速度与自旋按保留比例衰减，其余当作被软革吸收。
- 决策：取③。新增 `TablePhysics.pocketLinerRestitution=0`、`pocketLinerRetention=0.4`，`LocalPocketSimulation.Surface.tangentialRetention`（默认 1，不改其他面），`PocketContactResponse.linerSink` 在单球与多球两条冲量路径的刚体解之后施加。库鼻、台呢斜边、袋沿、捕获边界与固定回收尾段均不变；`pocketThroatRestitution` 只剩平面模型喉壁使用。
- 原因：①被 leather-sweep-r1 否决——μ 0.2–2 × e 0–0.45 共 16 组全部弹出、μ≥0.5 逐位相同（Coulomb 切向冲量至多把 2/7 滑动转成自旋，无法表达软革抓球）；②代价最大且实物显示皮革本就不是碰撞结构；③是刚体模型内最小的、有实物功能依据的表达，并保持「进袋与轨迹同源、不放大捕获区、不提前隐藏」约束。
- 验证：liner-retention-sweep-r1 保留 ≤0.6 八组全部落洞、1.0 两组弹出；liner-regression-r2（-O，与 arc-reach-regression-r1 同选择集）55 项 1 失败，原 10 失败方法中 9 个转绿、无新增；唯一失败为旧捕获圈窗口语义（球贴后壁落洞、距袋心 29.8 mm，仍在洞口 43 mm 内），测试窗口改为「球心在洞口内」。liner-regression-r4（专用模拟器）PocketBehaviorDiag/PocketRefactorDiag/PocketLeatherIntegration/ScoringOnlyConsistency 61 项 57 过 3 跳过 1 失败：`test_R2_railFrozenEndToEnd` 穿透 5–11 mm，A/B 证明旧参数逐位相同，为既有贴库穿透问题，与本决策无关。保留比例 0.4 是按实物功能标定的模型参数，不是实测皮革系数；真机性能、六袋偏入全矩阵仍属 W07/W16。

### ADR-P10-14 — 裁定平面、呈现空间：默认模拟模型切回平面判据（2026-09-14，本地验收）
- 场景：v63 W03 起 `appDefault` 为空间袋口模型，求解器每个候选都跑袋内刚体接触，满台预测中位 6.9 s、防守搜索 20 s 级；且 W07 常规进袋失败全部源于袋内物理。用户 2026-09-14 裁定：求解器沿用「球心进袋口圈即进」，袋内物理只做呈现。
- 选项：①继续以空间模型为裁定并优化性能；②搜索用平面、终验用空间（B4 曾因双景观错配被否）；③裁定全平面（`appDefault = .planarReference`），空间模型改显式 `spatialPockets`，仅在判进后用于单球呈现（W17-B）与确定性落位（W17-D），呈现段不得回写任一裁定字段。
- 决策：取③。理由：既有求解/评分/规则均按平面判据标定；③无双景观错配（裁定唯一真源）；DR-292 后内衬耗能，平面判进与空间落洞结论在常规力度下一致，呈现层不再需要「反悔」。性能 A/B：单杆 204→4 ms、满台 6898→27 ms、斯诺克 7.51→0.63 s。
- 代价与守护：v63 W05–W08 为空间袋口所做的调度/接管/收集边界降级为呈现层依赖，其回归测试改为显式模型；空间求解器既有的 `supportConvergence` 与 1e-12 级失败（PocketGeometryV63Tests 等）不再阻塞裁定路径，但 W17-B 使用该求解器前必须先闭合。H01–H04 腾空需「平面袋口 + 空间腾空」组合模型，列 H03 设计项。
- 验证与来源：DR-293、`tasks/3d-v63/W17-working.md`、`output/3d-v63/W17/`。

### ADR-P10-15 — 进袋呈现：脚本化网兜落位替代袋内空间求解（2026-09-14，本地验收）
- 场景：ADR-P10-14 后默认裁定为平面判进，3D 回放退回 pre-v63「吸到袋心再淡出」。用户要求进袋后有完整下落轨迹并停在网兜中，多球超容量先进先出。
- 选项：①判进后跑 `LocalPocketSimulation` 单球 ≤0.3 s 再脚本落位（方案 v63.13 W17-B 原文）；②全程脚本：重力 + 零恢复/切向保留 0.4 的内衬 + 资产实测网兜剖面 + 确定性槛点；③沿用吸到袋心。
- 决策：取②（DR-294）。①的求解器在袋底双接触形态有未闭合停滞且需在平面 App 内加载 2 s 的 USDZ 资产；②与①共用同一套物理常量、确定性、实时/导出同源，并且 tail 结构与来源无关，①可作后续替换。③不满足需求。
- 守护：呈现段只读 `pocketEntries`、只写 `PocketCollectionTail`，规则/评分零改动（`recordPlanarCollectionTail` 拒绝无判进记录的球）；剖面常量由测试对资产门禁；容量/FIFO 由固定用例保证。
- 未闭合：跨杆保留（页面层）、W03 裁剪例外、用户实看。来源：DR-294、`tasks/3d-v63/W17-working.md` §W17-B/D。
- 补记（DR-297，2026-09-14）：用户实看漏斗版后报穿模与慢落。壁模型改为对资产（皮革+库颚+网绳）实测的 27 环 × 12 方向极坐标球心自由区（角袋皮革杯轴在平面袋心内侧 24 mm、非漏斗）；接触改为「到达冲击整体切向 ×0.4，此后沿坡向仅重力+Coulomb、环向内衬握持、`v.y` 不乘任何系数」。球体侵入网格由 ≈48 mm 降到 1.0 mm，触底 0.19/0.25 s（自由落体 0.16 s）。决策②不变，守护新增两条对资产的门禁。
- 补记（DR-298，2026-09-14）：用户实看 DR-297 版后要求球「继续往下滚到支架底部」。实测内置 USDZ 为开口网兜 + 金属回球支架（`Black` 双杆沿内向 X 轴 23° 下斜、`Gold` 挡头），新增 `PocketRailProfile`；槛点改为支架上靠挡头的 3 球链（容量由几何派生），下落分「网兜 → 穿底环自由落体 → 着陆死停横向对中 → a = g·sinθ·5/7 滚下 → 挡头死停」三段。资产底环内径 ≈50 mm 比球小 2.5 mm，脚本按真实开口穿过（呈现层取舍，不改裁定）。门禁 `testRailProfileMatchesBundledRods` 对六袋杆/挡头/底环加载 USDZ 实核。

**DR-299 补记（2026-09-14，跨杆保留）**：用户口径「永远 16 颗球，进袋即进球库留在支架；同球放回球桌时支架里的它消失、后方按序前移」。支架球改为盘面节点克隆，由每场景 `PocketRailInventory` 驻留（`Core/Scene/PocketRailInventory.swift`）；`TrajectoryPlayback(railInventory:)` 布槛计入已驻留球、满链 FIFO 先淘汰旧杆球；`recorder.planarTailOccupancy` 保证求解预览先 attach 后场景回放仍能按真实占用重排。求解器 / 导出 / 规则 / 计分不受影响。详 `tasks/IMPLEMENTATION-LOG.md` DR-299。


### ADR-P10-16 — 支架库存与时间线由实时、导出、历史恢复共用（2026-09-14）

- 背景：DR-299 将已进袋球保留为场景克隆，但库存只存在于节点和 Timer 中，导出无法跨杆继承，撤销/取消也不能恢复在途节点。
- 决策：用户同意继续后，将稳定 boardKey、袋号、槽位、位置/朝向与链几何收为 `PocketRailSnapshot` 值状态。`PocketRailTimeline` 复制本杆派生轨迹，按模拟时刻绝对采样；预积旋转前缀避免每帧从头积分。预测别名 `object` 在消费边界映射回真实球号。裁定与内容 JSON 不变。
- 实时动作与离线逐帧导出共用轨迹与 FIFO；前者由盘面动作时钟采样，后者显式传模拟时刻，不启用库存 Timer。已退场克隆不得计入可见库存；仍在淡出的节点由库存持有以支持清空。开始/结束/取消有代次校验，移除动作后仍可恢复杆前库存。
- 历史边界：重打/撤销恢复杆前，回放结束恢复回放前，连续序列保存每杆库存；外部新局与整局重置清空，普通清桌保留支架球，返回桌面的同号球从支架移除。存档续编按现有当前引擎重算流程重建库存；JSON 未保存球面姿态，不能声称重现旧视频贴纸朝向。
- 验证：完整八杆实时起终盘面/支架与事件对照，30/60 fps × 1/0.5 倍速共 4638 个运动帧通过；稳定 ID、FIFO、快照恢复、取消与训练页回归详 `tasks/3d-v63/W17C-continuation.md`。近景与真机/用户实看另列，不以数据一致替代视觉验收。
- 范围：用户同日明确扎杆/跳球 H01–H04 延至下版，本 ADR 不启用抬杆输入或扩展其物理模型。


ADR-P10-16 真机补记（2026-09-15，FL-072）：SCNAction renderingQueue 和主线程共同访问库存，改为自有 NSRecursiveLock 串行化；正常杆末必须先 finishPlayback 再清动作/保存下杆快照，取消恢复杆前。原纯值快照/时间线与物理裁定不变。真机八杆四档4638运动帧、取消/迟到回调/释放/三页历史恢复6项通过；详 W16-device-20260914。
