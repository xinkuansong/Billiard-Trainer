...
## ADR-P2-20260906 — 启动与训练数据云同步选择

- 状态：用户已确认，实施完成；自动化验证结果见报告，真实 Apple/跨设备仍待真机验收。
- 决策：RootView 直接展示主界面，首次打开也不以引导/认证流程阻断。既有凭证在后台校验；没有有效登录时在「我的」提供登录入口。后台失败不弹账号恢复或登录过期提示。
- 云同步与认证/Pro 权益分开。每台设备按账号保存明确的开启/关闭选择，未选择视为关闭；首次主动登录提供选择，冷启动不主动弹窗。偏好设置可更改。
- 开启后同步当前账号的训练会话和角度成绩；游客数据须另行确认合并。拒绝合并不代表关闭云同步，文案必须说明。
- 关闭后保留本地记录、已有云端副本及待同步队列；不启动后续上传/下载，忽略尚未落盘的下载结果。关闭前已发出的服务端写请求无法撤回，不将开关宣称为删除云端数据。
- 并发：后台身份结果不得覆盖新的主动登录；旧请求 401/refresh 返回后校验凭证是否仍属于原请求，避免清空或覆盖新登录凭证。
- 兼容：不更改 SwiftData schema、数据 owner 或 Pro 资格规则；旧安装没有明确偏好时默认关闭，允许设置开启。
- 验证：定向 AuthState / AccountDataCoordinator / 上传删除与下行恢复回归，以及首次启动、设置开关跨启动保存 UI；见 `output/account-sync-choice/REPORT.md`（完成后填写）。


## ADR-P2-20260909 — 重新开启同步的恢复与反馈

- 背景：用户真机报告首次拒绝同步、退出重登后接受，只有头像昵称，训练等数据未显示。手机版本、当时请求日志及云端记录尚未取得，不能把某一代码缺陷认定为该次真机事件的唯一根因。
- 决策：保留按设备账号记录的同步同意；拒绝后不自动打开，设置提供明确的全量重试入口，并允许重新选择游客合并。账号身份由登录 sheet 的呈现者在 onDismiss 后提交，防止身份切换重建页面与同步提示竞争。
- 同步进行时前台增量触发不作废当前恢复；关闭/换号仍通过 generation、账号及同意版本检查阻断后续请求与旧响应落盘。上传应答返回后也重查授权，避免旧操作出队。
- 恢复服务返回训练/成绩失败结果；上传失败计数回传编排层。请求被拒绝的队列项保留，而非按旧 4xx 策略丢弃；只有成功才出队。已被旧版本丢弃的队列不会凭空恢复。
- 成功保存恢复数据或完成游客归属迁移后，按 owner 发出 didRestoreAccountData，历史、统计、首页重新读取本地数据。不开全局页面重建，不丢导航与训练草稿。
- 设置显示进行中/恢复条数/未完成状态，失败可重试。云同步范围仍为训练会话、角度与瞄准成绩；本轮未新增收藏/计划/安排云端服务，界面明确范围，游客本地归属迁移保持原有实体覆盖。
- 不变：SwiftData schema、DTO、后端端点、账号身份及 Pro 规则。未发布或操作真实账号。
- 验证与真机补验：tasks/ui-reviews/UR-20260909-account-sync-repair.md。
