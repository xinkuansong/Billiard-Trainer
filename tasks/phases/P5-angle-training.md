...
- [ ] `AngleCalculator.contactPointOffset(angle:)` → `sin(α) × R`，精度 ≤ 0.001
- [ ] `AngleCalculator.randomAngle(pocketType:)` → 角袋 5°–85°，中袋 15°–60°，以 5° 为步进
- [ ] `AdaptiveQuestionEngine` 实现加权出题：
  - 基础权重 30% + 误差历史权重 70%
...
- [ ] 顶视图球台（`BTBilliardTable`）展示：目标球（随机位置）、袋口方向标记
- [ ] 母球位置随机生成，位于合理击球区域（不贴近袋口）
- [ ] 数字键盘输入估计角度（0–90°整数）
- [ ] 「确认」按钮提交答案
- [ ] 每题显示题号（如「第 5 / 20 题」）
...
- [ ] 提交后动画：正确击球路线（绿色）高亮，用户答案路线（橙色）对比展示
- [ ] 目标球面标注红点（正确接触点位置）
- [ ] 显示：用户答案、正确答案、误差值（±X°）
- [ ] 误差 ≤ 3° 显示绿色「精准」，3°–10° 显示橙色「接近」，>10° 显示红色「偏差较大」
- [ ] 「下一题」按钮，动画消退后出下一题
...
- [ ] 每次提交答案后，`AngleTestResult` 写入 SwiftData
- [ ] 角度 Tab 内「历史」入口：显示近期平均误差趋势（折线图，每组 5 题）
- [ ] 分别展示角袋/中袋误差趋势
- [ ] 历史数据在 App 重启后保持
...
- [ ] 交互式滑块（0°–90°，步进 1°）：拖动时目标球示意图实时更新接触点位置
- [ ] 数值显示：当前角度 + 偏移百分比（sin(α)×100%）
- [ ] 完整静态对照表（13 个标准角度：0°/10°/15°/20°/25°/30°/35°/40°/45°/48.6°/60°/75°/90°）
- [ ] 每行含小型球面示意图（标注接触点）
- [ ] 原理说明：简要解释 `偏移 = sin(α) × R`
...
- [ ] 免费用户每日测试上限：20 题
- [ ] 达到上限时显示引导订阅提示（`BTPremiumLock` 变体）
- [ ] 每日计数在本地重置（通过日期比较，不依赖服务器）
- [ ] 付费用户无限制
