...
- **分享同时保存 + 浅色背景（2026-08-27，SwiftUI Developer，DR-079）**：生成分享图先落库；分享卡默认浅色，可选炭灰等。聚焦单测 9/0。未开模拟器点分享页。
- **本项心得回显（2026-08-27，SwiftUI Developer）**：历史详情动作卡 + 总结页明细回显 `DrillEntry.note`；空/纯空白不上屏。分享图与编辑仍只对整场心得。聚焦单测 4/0。未开模拟器点历史详情。
- **休息最小化计时改回金色（2026-08-27，SwiftUI Developer）**：收起后右下计时胶囊改回 `btAccent`，与绿色主按钮区分。卡顶「最小化」仍用浅绿提亮。未开模拟器。
- **休息卡最小化按钮提亮（2026-08-27，SwiftUI Developer）**：卡顶「最小化」由灰底灰字改为 `btPrimary` + `btPrimaryMuted`，避免像禁用。未开模拟器。
- **休息最小化小图标只在记分页（2026-08-27，SwiftUI Developer）**：收起后记分页右下金色计时胶囊；跨 Tab 浮标不再显示组间休息。聚焦单测 4/0。未开模拟器。
- **休息卡最小化与会话脱钩（2026-08-27，SwiftUI Developer，FL-033）**：休息卡只收覆层；底栏才整场离页；回来展开覆层。撤回训练 Tab 挂会话。聚焦单测 4/0。未开模拟器点休息卡。
- **最小化留在记分页（2026-08-27，SwiftUI Developer）**：⚠️ 返工（见 FL-033）训练 Tab 挂会话是误判。
