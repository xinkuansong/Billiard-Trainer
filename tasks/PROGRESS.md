...
- **问题集合 v47.1 整体 UI 重设计 + Stitch 裁定（2026-08-30，Orchestrator）**：用户确认四页只是试点，现进入全页面 UI 重新设计。使用 Stitch MCP，但旧 `QiuJi Angle & Strike View` / `Chalk & Slate` 保持只读，W1a 新建 v47 专用项目；先做六类 Light/Dark 母版，点验后再翻译 SwiftUI。修正基线为实际 66 张，补哈希/缺图门禁/几何全触发，拆为 W0–W15 可验收批次。尚未改 SwiftUI、未创建新 Stitch 项目。真源 `问题集合_v47.md` v47.1。
