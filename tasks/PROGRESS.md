...
- **问题集合 v38 W7 ✅ / 全案收官（2026-08-14，v38.10，Orchestrator + Content Engineer）**：全能 c066→c064→c068→c069→c071→c067→c065→c070；咬合 c082/c054/c078 各 1 次。I13 引入序对照 W0（只比序）；c069/c071 首次引入挤进第 1 周。货架 12 份，准度Ⅲ Pro。证据：`make build` SUCCEEDED；`verify-gate` FAIL 0（I11 270 / I13 428）；构造性 `i13_intro_order_vs_w0` exit 1；聚焦单测 27/27；UI `V37W4PlanShelfUITests` 1/1。截图：货架、准度Ⅰ第一堂近台 c011、走位Ⅱ第一堂半台 c080。契约 **2.11**。未 commit。
- **问题集合 v38 W6 ✅（2026-08-14，v38.9，Content Engineer）**：基本功手架回 W1、五分点在定杆后；力度 c046 移出强弱杆周；特殊球翻角袋先于 K 球、禁中袋热身伺候角袋。I13：c023 首次引入提前到第 2 周；c060/c061 挤进第 3 周。证据：`make build` SUCCEEDED；`verify-gate` FAIL 0（I11 268 / I13 415）；V21W5 10/10 + V22W3 2/2；ReadLints 0。抽检 c046 sequence 8→6 链长 5；c006 4×12。未 commit。**下一动作 W7**。
- **问题集合 v38 W5 ✅（2026-08-14，v38.8，Content Engineer）**：走位Ⅰ序未改、改末日热身；走位Ⅱ半台/两库先于蛇彩，c040 focused。课时 120′/150′。证据：`make build` SUCCEEDED；`verify-gate` FAIL 0（I11 265 / I13 414）；`V21W5PlanContentTests` 8/8；ReadLints 0。抽检 sequence 衰减 c079 8→6 杆数 4 不变；repetition c034 5×12、c040 10×12。未 commit。**下一动作 W6**。
- **问题集合 v38 W4 ✅（2026-08-14，v38.7，Content Engineer）**：分离角 c024→c026→c025→c084→c027→c028→c029→c030→c031→c083；开档咬合 c032。I13：c025 scalar=4，后 7 条首次引入挤进第 1 周，W2–W4 只做衰减复现。课时 120′。证据：`make build` SUCCEEDED；`verify-gate` FAIL 0（I11 262 / I13 411）；聚焦 2/2；ReadLints 0。抽检 repetition 衰减 rounds=defaultRounds、bpr 下降（c024 6×12、c084 8×12）。本线程无 sequence。未 commit。**下一动作 W5–W6**。
- **问题集合 v38 W3 ✅（2026-08-14，v38.6，Content Engineer）**：杆法Ⅰ c016→c014→c003→c004→c015→c017；杆法Ⅱ c073→c074→c075→c018→c020→c021，禁带塞。I13：c018/c020/c021 首次引入提前到第 3 周（scalar 2 不能接在 c075=3 的下一周）。证据：`make build` SUCCEEDED；`verify-gate` FAIL 0（I11 254 / I13 405）；聚焦 26/26。未 commit。**下一动作 W4–W6**。
- **问题集合 v38 W2 ✅（2026-08-14，v38.5，Content Engineer）**：准度Ⅰ/Ⅱ/Ⅲ 按 W0 重排。引入序近台先于半台、c032 先于 c002；准度Ⅱ无带塞；准度Ⅲ c076→c077→c078。证据：`make build` SUCCEEDED；`verify-gate` FAIL 0（I11 252 / I13 403）；聚焦 27/27；ReadLints 0。抽检 repetition 衰减 rounds=defaultRounds、bpr 下降（c001 5×12、c078 16×10）。本线程无 sequence。未 commit。
- **问题集合 v38 W0 ✅ / W1 ✅（2026-08-14，v38.4，Orchestrator）**：W0 过目通过（§0 五条维持）。W1：`FormationRounds.ballsPerRound` + Resolver 按 mode 分叉（repetition 衰减保位置、降颗数；sequence 降遍数）；I11 收紧；存量 repetition 衰减机械迁移；`index.json` v8 + `plan_accuracy3` 槽（folder reference，yml/pbxproj 未改行）。I13 外键：准度Ⅲ W1 三堂占位 c076/c077/c078 完整剂量；全能 c078 `reviewFrom` → `plan_accuracy3`。证据：`make build` SUCCEEDED；`verify-gate` FAIL 0（I11 236 / I13 374）；构造性 39/39（含砍位置 / 改链长 / 合法降遍数）；聚焦单测 33/33。契约 **2.10**。未 commit。
- **问题集合 v37 W5 ✅ / 全案收官（2026-08-14）**：I13 排课门禁接入 `verify-gate` / pre-push。操作定义：focused 首次引入 scalar 不降；非咬合热身 scalar ≤ 主课（`reviewFrom` 热身豁免）；衰减球数单调不增；咬合外键有效。I13 通过 382 / 违规 0；构造性 4/4；自测 36/36；`make verify-gate` FAIL 0；`make build` SUCCEEDED；聚焦 43/43；UI 3/3。截图亲验货架/Freemium/雷达三形。契约 **2.9**，真源 **v37.11**。**已提交 `7d229fa`。** R4 教学序未收口 → v38。
