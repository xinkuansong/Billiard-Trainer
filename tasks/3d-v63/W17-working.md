...
## DR-300 — 支架着陆滑→滚角动量守恒（2026-09-14）
用户觉得支架滚动稍快。探针证实资产为「中托 + 双护」三杆、滚动半径 = R，5/7 因子无误；错的是着陆时保留全部切速并瞬时赋纯滚自旋。改为 v_roll = 5/7·v_t + 2/7·R·ω_t（无自旋落下损失 2/7 起滚速度）。全程 +30 ms；末速仍由 23° 斜面决定。15/15 通过。剩余自由度：资产斜度、有来源的滚动阻力系数。
