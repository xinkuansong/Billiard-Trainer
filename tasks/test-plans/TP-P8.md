...
| V-15 | TrainingGoalView | 不显示「每次训练时长目标」及不限/分钟档位；真实训练时长仍只在历史与统计中呈现 | [ ] |
...
| V-32 | AboutView | H-09 完成前明确显示“用户协议与隐私政策尚未发布”，不得展示测试网址或无响应链接；发布后再验收两个正式 HTTPS 入口 | [ ] |
...
| D-07 | AboutView | QJ 绿块、各行图标底、法律发布状态提示在深色下可读；H-09 完成后再核链接态 | [ ] |
...
### 流程 2：TrainingGoal — 周目标 + 提醒

**步骤**：
1. 「我的」→「训练目标」
2. 修改「每周训练天数」为不同值，观察环上「/ N 天」与中间数字联动
3. 确认页面中不再出现「每次训练时长目标」、不限或分钟档位
4. 打开「开启提醒」，调整「提醒时间」；再关闭 Toggle

**预期结果**：周目标立即生效；无训练时长目标设置；提醒区展开/收起带动画；时间可选。

- [ ] 修改每周天数后进度环比例与文案「X / N」一致（X 为本周有记录的去重天数）
- [ ] 本月达成率显示百分比或合理占位（月初等边界见第五节）
- [ ] 不显示时长目标标题及 6 档选项；既有训练记录的真实时长未被改写
...
### 流程 6：AboutView — 反馈、评分与法律发布状态

**步骤**：
1. 「我的」→「关于与反馈」
2. 点「意见反馈」→ 应尝试打开 `mailto:feedback@qiuji.app`（模拟器可能无邮件客户端）
3. 点「给个好评」→ 触发系统评分请求（`requestReview`，可能不每次弹出）
4. H-09 未完成时，核对法律区明确显示“尚未发布”且不存在可点击的伪入口；H-09 完成后，点「用户协议」「隐私政策」并核对 Safari 打开最终 URL

**预期结果**：mailto 构造正确；法律状态与 H-09 事实一致；无空操作、测试网址或崩溃。

- [ ] mailto 含主题与设备信息后缀（真机有邮箱客户端时）
- [ ] 评分请求不崩溃（无弹窗亦可能为系统策略）
- [ ] H-09 未完成时显示诚实占位；完成后两个公开 HTTPS 链接均可打开且无证书警告
...
| I-03 | TrainingGoal 列表 | 7 个周目标天数滚动流畅，快速切换选中无卡顿；不存在时长目标档位 | [ ] |
...
| I-07 | About 行 | 意见反馈 / 好评整行响应；H-09 完成前法律提示不可误导为可点击入口，完成后链接整行响应 | [ ] |
...
| E-02 | PersonalInfo 游客 | 游客完成引导后进入个人信息：无「账号绑定」；昵称与头像只保存到稳定 guest owner，不生成伪后端账号 | [ ] |
...
| E-09 | 训练提醒 | 拒绝权限时不排程并显示可理解提示；允许后存在每日 pending request；改时替换旧请求；关闭后取消；真机实际到点通知由 H-26 留证 | [ ] |
