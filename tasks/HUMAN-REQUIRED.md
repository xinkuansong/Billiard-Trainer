...
- **本轮改动范围**：仅修改上述 3 条的 `description`、`coachingPoints`、`standardCriteria`；未改名称、摆球、动画、教程、训练剂量和过关数字。c012 已按全部逐杆图明确为 8 个纵向摆位、向同一中袋改变整条瞄准线方向；彩球只作摆位标记。
- **待后续处理的一致性问题**：c012 教程正文仍写“只拉距离、不换切角”，与训练意图及 8 张逐杆图不符；本轮按试写边界未修改教程。
- **尚未逐图复核**：drill_c013、c032、c073、c074、c075，仍是待复核样稿，不能作为扩量样板。
- **自动检查**：3 份 JSON 解码、`verify-copy`、`git diff --check` 与 Ruby YAML 回退校验已通过。官方 `quick_validate.py` 两次均因当前 Python 缺少 PyYAML 而未能启动，属于校验器运行时依赖缺失，不把它记作通过。
- **请重点核查**：① 遮住标题后，三条文案是否仍能一眼区分；② 提示是否像台边下一球能执行的话；③ 达标标准是否足够短且没有丢掉原过关要求。
- **通过后**：按动作类别分组扩展到其余动作库与练习卡片，不跨组批量套模板。
