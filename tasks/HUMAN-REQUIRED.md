...
- **做什么**：在模拟器或真机上逐条执行以下 6 份测试计划，通过则勾选 `[x]`，失败则记录问题：
  | 文件 | 覆盖范围 | 预计时长 |
  |------|---------|---------|
  | `tasks/test-plans/TP-P2.md` | 数据层、离线、登录迁移 | 30 分钟 |
  | `tasks/test-plans/TP-P3.md` | 动作库列表、详情、收藏、锁定 | 30 分钟 |
  | `tasks/test-plans/TP-P4.md` | 训练记录全流程、自定义计划、分享 | 45 分钟 |
  | `tasks/test-plans/TP-P5.md` | 角度测试、对照表、每日限制 | 30 分钟 |
  | `tasks/test-plans/TP-P6.md` | 历史日历、统计图表、60 天限制 | 30 分钟 |
  | `tasks/test-plans/TP-P7.md` | 订阅购买、恢复、过期降级 | 30 分钟 |
- **在哪里**：Xcode → iPhone 16 Pro Simulator（或真机）
- **预计时长**：总计约 3 小时
- **影响任务**：T-P8-12、QA-P8 最终验收
- **完成后**：在每份 TP 文件底部「测试结果」区填写结论；如有问题填写「发现的问题」表。全部完成后将本条改为 ✅。
