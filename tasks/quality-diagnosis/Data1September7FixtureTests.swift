...
/// Uncompiled diagnostic draft. Fixed ledger: 2026-09-07, Asia/Shanghai.
/// Dedicated empty guest installation only. Never register/run automatically.
@MainActor
final class Data1September7FixtureTests: XCTestCase {
    ...
    func testSeedSevenSessionLedgerIntoDedicatedEmptyDefaultStore() throws {
        ...
        let manifestRelativePath = "Documents/qd-data1-20260907-manifest.json"
        ...
        try require(formatter.string(from: Date()) == "2026-09-07", "Fixed ledger expired: prepare a NEW independently calculated ledger; do not change system clock")
        ...
        try require(Date() >= date(9, 7, 3), "Ledger must contain no future sessions")
        ...
        func row(_ id: String, _ kind: String, _ when: Date, _ minutes: Int, _ entries: [DrillEntry] = []) -> TrainingSession {
            ...
            s.note = "QD-DATA1-0907-" + id; s.drillEntries = entries
            context.insert(s); return s
        }
        _ = row("A", "drill", date(9, 7, 1), 20, [entry(8, 10), entry(2, 5, "球", 1)])
        _ = row("B", "drill", date(9, 7, 2), 30, [entry(1, 2, "局")])
        let cognitive = row("C", "cognitive", date(9, 6), 5)
        ...
        answer.date = date(9, 6, 1); context.insert(answer)
        _ = row("D", "tool", date(9, 7, 3), 99)
        _ = row("E", "drill", date(9, 1), 10, [entry(3, 10)])
        _ = row("F", "drill", date(8, 31), 40, [entry(4, 10)])
        _ = row("G", "drill", date(8, 6), 80, [entry(6, 10)])
        try context.save()
        ...
        try require(Set(rows.map(\.note)) == Set(["A","B","C","D","E","F","G"].map { "QD-DATA1-0907-" + $0 }), "Ledger identity mismatch")
        ...
        try require(formatter.string(from: Date()) == "2026-09-07", "Midnight crossed: invalid fixture")
        let manifest: [String: Any] = [
            "fixture": "DATA1-20260907-v1", "localDay": "2026-09-07", "timezone": "Asia/Shanghai",
