...
## 2026-09-08 现场观测准备（未执行）

主控已全文审阅PerformanceDiagnosticUITests及RepeatedShotDiagnosticUITests，计划分别选择5次进程启动与10轮击球/回放/重打伴随方法；不再重复10次只进出工具来代替真实回放。新run_observed_ui_batch.py包装原串行runner，在匹配QD004专用已启动设备上先确认simctl录屏开始，再启动测试；每秒仅筛选该UDID App容器的精确可执行路径，记录PID/RSS与单调时间、负载。结束只向本脚本创建的录像进程发送SIGINT。录屏、进程数据和测试结果独立保存；尚未实际运行，也未取得任何性能数值。

脚本py_compile通过，不代表现场录屏或PID筛选已验证。若没有匹配PID，明确采样缺失，不用其他App的进程顶替。原测试JSON时刻与连续视频需对齐；10轮每次击球/回放都必须逐段图审并留帧定位，终态按钮可用或单张静态图不能证明运动。计划的5次启动仍含XCTest开销、内存空库、跳引导/模拟Pro，不是Release/真机冷启动SLA。
