...
override PROJECT_ROOT := $(QD_REPO_ROOT)/build/quality-diagnosis/snapshot-004
QD_RELEASE_OUTPUT ?= $(QD_REPO_ROOT)/build/quality-diagnosis/formal-004-release-001
...
qd-release:
	...
	@test "$(SCHEME)" = "QiuJi"
	@case "$(QD_RELEASE_OUTPUT)" in "$(QD_REPO_ROOT)/build/quality-diagnosis/"formal-004-release-*) ;; *) echo "Refusing output outside diagnostic Release runs" >&2; exit 2;; esac
	@test ! -e "$(QD_RELEASE_OUTPUT)" || { echo "Existing run: inspect it; use a new output suffix only for an intentional new run" >&2; exit 2; }
