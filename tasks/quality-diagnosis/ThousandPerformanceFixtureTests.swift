...
@MainActor
final class ThousandPerformanceFixtureTests: XCTestCase {
    func testSeedDedicatedEmptyDefaultStoreForNormalHistoryUI() throws {
        ...
        guard let device = setting("QD_EXPECTED_DEVICE_UDID"), UUID(uuidString: device) != nil,
              env["SIMULATOR_UDID"]?.lowercased() == device.lowercased() else {
            XCTFail("Dedicated device mismatch; no write")
            throw NSError(domain: "QDThousandFixture", code: 2)
        }
        guard setting("QD_ALLOW_THOUSAND_DISK_SEED") == "DEDICATED_EMPTY_GUEST_SIMULATOR" else {
            XCTFail("Explicit dedicated empty guest simulator authorization is absent")
            throw NSError(domain: "QDThousandFixture", code: 1)
        }
        ...
        let expectedOwner = try XCTUnwrap(setting("QD_EXPECTED_GUEST_OWNER"))
        let expectedDay = try XCTUnwrap(setting("QD_EXPECTED_LOCAL_DAY"))
