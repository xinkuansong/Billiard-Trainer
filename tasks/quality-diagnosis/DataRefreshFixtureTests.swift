...
/// Uncompiled diagnostic draft. Fixed ledger: 2026-09-08, Asia/Shanghai.
/// Dedicated empty guest installation only. Never register/run automatically.
@MainActor
final class DataRefreshFixtureTests: XCTestCase {
    ...
    func testSeedThreeSessionRefreshLedgerIntoDedicatedEmptyDefaultStore() throws {
        let env = ProcessInfo.processInfo.environment
        func setting(_ key: String) -> String? { env[key] ?? env["TEST_RUNNER_" + key] }
        try require(setting("QD_ALLOW_DATA_REFRESH_DISK_SEED") == "DEDICATED_EMPTY_GUEST_SIMULATOR", "Explicit DATA-REFRESH authorization missing")
        ...
        try require(!args.contains("-forcePremium"), "Seed host may not use purchase override")
        let owner = try XCTUnwrap(setting("QD_EXPECTED_GUEST_OWNER"))
        ...
        let expectedRelativePath = try XCTUnwrap(setting("QD_EXPECTED_DEFAULT_STORE_RELATIVE_PATH"))
        let manifestRelativePath = "Documents/qd-data-refresh-20260908-manifest.json"
        ...
        try require(formatter.string(from: Date()) == "2026-09-08", "Fixed ledger expired: prepare a NEW independently calculated ledger; do not change system clock")
        ...
        try require(Date() >= date(9, 8, 2), "Ledger must contain no future sessions")
        ...
        func row(_ id: String, _ kind: String, _ when: Date, _ minutes: Int, _ entries: [DrillEntry] = []) -> TrainingSession {
            ...
            s.note = "QD-DATA-REFRESH-0908-" + id; s.drillEntries = entries
            context.insert(s); return s
        }
        _ = row("A", "drill", date(9, 8, 1), 20, [entry(8, 10), entry(2, 5, "球", 1)])
        _ = row("B", "drill", date(9, 8, 2), 30, [entry(3, 5)])
        _ = row("C", "drill", date(8, 31), 40, [entry(4, 10)])
        try context.save()
        let rows = try context.fetch(FetchDescriptor<TrainingSession>())
        try require(rows.count == 3 && rows.allSatisfy { $0.ownerKey == owner }, "Persisted session count/owner mismatch")
        try require(Set(rows.map(\.note)) == Set(["A","B","C"].map { "QD-DATA-REFRESH-0908-" + $0 }), "Ledger identity mismatch")
        try require(try context.fetchCount(FetchDescriptor<DrillEntry>()) == 4, "Entry count mismatch")
        try require(try context.fetchCount(FetchDescriptor<DrillSet>()) == 4, "Set count mismatch")
        let answers = try context.fetch(FetchDescriptor<AngleTestResult>())
        try require(answers.isEmpty, "Cognitive relationship mismatch")
        try require(try context.fetchCount(FetchDescriptor<SyncPendingItem>()) == 0, "Unexpected sync queue")
        try require(formatter.string(from: Date()) == "2026-09-08", "Midnight crossed: invalid fixture")
        let manifest: [String: Any] = [
            "fixture": "DATA-REFRESH-20260908-v1", "localDay": "2026-09-08", "timezone": "Asia/Shanghai",
            ...
            "manifestRelativePath": manifestRelativePath, "owner": owner, "sessionCount": 3, "entryCount": 4,
            "setCount": 4, "answerCount": 0,
