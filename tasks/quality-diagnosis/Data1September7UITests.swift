...
final class Data1September7UITests: XCTestCase {
    ...
    private func checkDay() throws {
        ...
        try require(TimeZone.current.identifier == "Asia/Shanghai" && f.string(from: Date()) == "2026-09-07", "Fixed ledger day/timezone expired")
    }
    override func setUpWithError() throws {
        ...
        try require(m["fixture"] as? String == "DATA1-20260907-v1", "Wrong ledger")
        try require(m["localDay"] as? String == "2026-09-07" && m["timezone"] as? String == "Asia/Shanghai", "Wrong ledger calendar")
    ...
    private func capture(_ stage: String) throws {
        let stem = "data1-0907-\(name)-\(stage)-\(runID)"
    ...
    func testNormalGuestHomeTodayHistoryAndEntryCount() throws {
        ...
        let summary = app.descendants(matching: .any).matching(NSPredicate(format: "label == %@", "本周训练 1 / 3 天，连续训练 2 天")).firstMatch
        ...
        _ = try historyRow(["QD-DATA1-0907-D", "99 分钟"])
        let a = try historyRow(["半台直线球", "20 分钟"]); a.tap()
        let note = app.staticTexts["QD-DATA1-0907-A"].firstMatch; try reveal(note)
    ...
    // Focused continuation: actual tool history is a noninteractive static row.
    // Preserve the original button-based failure and avoid repeating home/count proof.
    func testStaticToolHistoryAndTwoEntryRecordReadback() throws {
        app.switchTab(.history)
        let tool = app.staticTexts["QD-DATA1-0907-D"].firstMatch
        let minutes = app.staticTexts["99 分钟"].firstMatch
        let kind = app.staticTexts["工具使用"].firstMatch
        try reveal(tool); try ready(minutes); try ready(kind)
        try require(abs(minutes.frame.midY - kind.frame.midY) < 12,
                    "Tool duration must share its kind row")
        try require(minutes.frame.minY >= tool.frame.maxY - 4 && minutes.frame.minY - tool.frame.maxY < 40,
                    "Tool duration must be directly below the unique tool title")
        try capture("static-tool-row-bound")
        let matches = app.buttons.matching(NSPredicate(format: "label CONTAINS %@ AND label CONTAINS %@", "半台直线球", "20 分钟"))
        try require(matches.count == 1, "Unique two-entry history row required")
        let a = matches.firstMatch; try reveal(a); a.tap()
        let note = app.staticTexts["QD-DATA1-0907-A"].firstMatch; try reveal(note)
        try capture("focused-A-note")
        for _ in 0..<8 { if visible(app.staticTexts["8/10"].firstMatch) { break }; scroll(false) }
        try ready(app.staticTexts["8/10"].firstMatch)
        try ready(app.staticTexts["2/5"].firstMatch); try capture("focused-A-two-entries")
        let back = app.navigationBars.buttons.firstMatch; try ready(back); back.tap()
        app.switchTab(.training)
        let summary = app.descendants(matching: .any).matching(NSPredicate(format: "label == %@", "本周训练 1 / 3 天，连续训练 2 天")).firstMatch
        try ready(summary); try capture("focused-home-returned"); try checkDay()
    }
