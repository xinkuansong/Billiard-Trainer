...
/// Observation only: normal c001 poster tap -> full PNG/AX. No zoom, paging or unknown close.
/// A green method means evidence collection finished, NOT full viewer acceptance.
final class ImageViewerObservationDiagnosticUITests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        let leaf = URL(fileURLWithPath: root, isDirectory: true)
            .appendingPathComponent("image-viewer-observation-" + UUID().uuidString, isDirectory: true)
    ...
    func testObservationNormalC001PosterOpensGalleryForAXReview() throws {
        ...
        do {
            ...
            try tap(poster) // Exactly one normal poster tap. No fabricated viewer route.
            ...
            // No assumed close/media AX identity; raw evidence supports the next reviewed method.
            let note: [String: Any] = [
                "classification": "observation-only", "device": expectedDeviceUDID,
                "entry": "normal library search -> c001 detail -> tutorial -> initial poster",
                "posterTapCount": 1, "uptime": ProcessInfo.processInfo.systemUptime,
                "expectedSourceGalleryCount": 6, "conservativeBottomExclusionBand": bottomExclusionBand,
                "requiredReviewerChecks": ["actual full-screen gallery", "1 / 6 and first caption", "visible media geometry", "actual close button AX"],
                "coverageVerdict": "pending actual PNG/AX review; no zoom/paging/return tested"
            ]
            let target = try XCTUnwrap(output).appendingPathComponent("observation-contract.json")
            try JSONSerialization.data(withJSONObject: note, options: [.prettyPrinted, .sortedKeys])
                .write(to: target, options: .withoutOverwriting)
        } catch {
