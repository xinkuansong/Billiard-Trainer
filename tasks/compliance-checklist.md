...
| 数据类型 | 是否收集 | 用途 | 是否关联用户 |
|---------|---------|------|------------|
| 用户 ID（LeanCloud objectId） | ✅ | App 功能（账号同步） | ✅ |
...
- [ ] 手机号仅用于登录身份验证，不用于营销
- [ ] LeanCloud 用户数据仅存储训练相关内容

### 2.3 用户权利保障

- [ ] App 内有「账号注销」功能（T-P8-06）：注销后删除 LeanCloud 云端数据
- [ ] 隐私政策中说明用户可申请数据访问和删除
- [ ] 提供联系方式（邮箱）供用户行使数据权利

### 2.4 数据本地化

- [ ] LeanCloud 服务节点选择「华北 · 节点 1」（数据存储在中国大陆）
...
- [ ] **数字内容必须使用 Apple IAP**：订阅功能不通过任何第三方支付（LeanCloud、微信支付等）
