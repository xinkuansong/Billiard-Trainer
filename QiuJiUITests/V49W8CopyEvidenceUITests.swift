...
/// v49 W8：三组加塞训练的详情/精讲 Light / Dark 取证。
final class V49W8CopyEvidenceUITests: XCTestCase {
    ...
    func testW8_Light_C018() { captureTutorial("drill_c018", "加塞一库变线", "固定摆位和进袋线路", "Light") }
    func testW8_Dark_C018() { captureTutorial("drill_c018", "加塞一库变线", "固定摆位和进袋线路", "Dark") }
    func testW8_Light_C020() { captureTutorial("drill_c020", "高杆加塞走位", "母球跟进后的第一碰库点", "Light") }
    func testW8_Dark_C020() { captureTutorial("drill_c020", "高杆加塞走位", "母球跟进后的第一碰库点", "Dark") }
    func testW8_Light_C021Detail() { captureDetail("drill_c021", "低杆加塞走位", "先看碰库前的回拉线路", "Light") }
    func testW8_Dark_C021Detail() { captureDetail("drill_c021", "低杆加塞走位", "先看碰库前的回拉线路", "Dark") }
    ...
    private func captureTutorial(_ id: String, _ title: String, _ focus: String, _ suffix: String) {
        ...
        save(app, "w8-\(id)-tutorial-\(suffix)")
        for _ in 0..<3 { app.swipeUp(); usleep(200_000) }
        save(app, "w8-\(id)-tutorial-later-\(suffix)")
    }
    ...
    private func save(_ app: XCUIApplication, _ name: String) {
        ...
        do { try shot.pngRepresentation.write(to: url); print("[V49-W8-SCREENSHOT] \(url.path)") }
