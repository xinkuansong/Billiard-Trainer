...
### ADR-003 — 自定义训练计划 SwiftData Schema 扩展（2026-04-02）

**背景**：T-P4-09 要求用户可新建自定义训练计划，需要持久化用户创建的计划和 Drill 配置。现有 `UserActivePlan` 仅支持指向 Bundle JSON 的官方计划 ID。

**决策**：
1. 新增 `CustomPlan`（name, sessionsPerWeek, createdAt）和 `CustomPlanDrill`（drillId, drillNameZh, sets, ballsPerSet, order）两个 `@Model`，cascade 删除关系。
2. `UserActivePlan` 新增 `isCustom: Bool` 字段（默认 `false`），当 `isCustom == true` 时 `planId` 存储 `CustomPlan.id` 的 UUID 字符串。
3. `TrainingHomeViewModel.load()` 根据 `isCustom` 分支加载官方或自定义计划。
4. 自定义计划无 week/day 结构，每次训练展示计划中全部 Drill。

**影响**：
- `ModelContainerFactory.allModels` 新增 `CustomPlan.self`、`CustomPlanDrill.self`
- SwiftData 自动轻量迁移（新增 model + 新增属性带默认值）
- 不影响已有 `TrainingSession` / `DrillEntry` 数据

**替代方案考虑**：
- 将自定义计划序列化为 JSON 存文件系统 → 拒绝，不利于 SwiftData 查询与 Sync 扩展
- 复用 `OfficialPlan` 结构写入 Bundle → 拒绝，Bundle 为只读
