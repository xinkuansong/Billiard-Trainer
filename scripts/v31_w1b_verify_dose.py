#!/usr/bin/env python3
"""v31 W1b 复验（只读）：对已写盘的 40 条 drill 做 I6a/I6b 预演 + 护栏 + 「只动两处」证明。
...
- V4（护栏 §5.6.3）：总量 Σ(ballsPerRound × defaultRounds) ∈ [40, 60]；
  综合类按局/按次的越界必须在 `GUARDRAIL_EXEMPT`（apply 脚本）中逐条附理由，否则 FAIL；
...
from v31_w1b_apply_dose import GUARDRAIL_EXEMPT  # noqa: E402
...
def main() -> None:
    ...
    for cat, drill_id in batch:
        ...
        if 40 <= total <= 60:
            ...
            guard = "越界·已豁免"
            exempted.append(f"{drill_id} 总量 {total}：{GUARDRAIL_EXEMPT[drill_id]}")
        else:
            guard = "越界"
            fails.append(f"V4 {drill_id}: 总量 {total} 不在 40–60 且无豁免理由")
    ...
    if exempted:
        print(f"\n--- 护栏豁免 {len(exempted)} 条（§5.6.3 综合类按局/按次）---")
        for e in exempted:
            print("  ⚠️ " + e)
    print(f"\nFAIL {len(fails)}")
    for f in fails:
