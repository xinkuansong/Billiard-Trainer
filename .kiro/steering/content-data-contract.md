...
> **版本**：2.19（2026-09-14：v63 裁定与呈现数据边界）
> **最后更新**：2026-09-14
...
2026-09-09（ADR-P2-20260909）：设置可主动全量重试并重新询问游客合并；运行中的恢复不被前台增量触发作废。上传拒绝/失败须保留队列并反馈，下载/保存失败不得报告成功。恢复落盘后按 owner 通知页面重读数据。
...
依据 `tasks/phases/P10-physics-content-pipeline.md` ADR-P10-14/15（2026-09-14）：

- 默认求解采用平面判据；进袋时刻、球标识、袋标识及规则结果同源于平面记录 `pocketEntries`。显式 `spatialPockets` 保留独立回归，不得把其捕获断言套在默认模型上。
- 网兜下落、支架落位及先进先出属于派生呈现，只能追加运动区间，不能反向改变 `objectPocketed`、进袋事件、评分或规则结果。支架克隆节点不是新增业务球。
- 对有完整可行求解的序列，实时与导出杆末采用预测终位；下一杆仍使用作者保存的 `before`。保存的 `after` 与当前求解不一致时，不能回写内容或强制判进来消除差异；无可行解时保留明确提示的保存结果跳过分支。
- 实时与导出须使用相同的裁定与派生呈现语义；跨杆支架库存的导出接入仍属 W17 未完成项，不能因单杆尾迹一致而宣布全部同源验收。
...
| I11 | 官方计划可解析：每条目 `drillId` 存在于 `index.json`、`dose` 结构可解析（`roundsPerFormation` 与 `formations` 恰好二选一、轮数 ≥1）、按球形引用的 token 存在于该 drill 的 `perFormation` token 集合 ∩ 序列 token 集合。**可调维按 mode（v38 R7）**：`repetition` 的 `rounds` 不得低于内容 `defaultRounds`（decay 也不砍位置）；`sequence` 仅在 `decay == true` 时允许 rounds < defaultRounds；`repetition` 可用可选 `ballsPerRound` 降每位置颗数（须 decay 且 ≤ 内容）；`sequence` 禁止改 `ballsPerRound`。**v44 ritual 白名单（只此一条）**：当且仅当 `dose.ritual == true` 且 `drillId == drill_c023` 且 `reviewFrom == plan_beginner` 且 `rounds ∈ {4,6}` 且 `ballsPerRound` ≤ 内容，允许 rounds < defaultRounds，且该计划内首次允许 decay。禁止推广到任意 repetition | I11（v31 W4；v34 下限；v37 decay 例外；**v38 W1 按 mode 分叉**；**v44 ritual 白名单**） | ✅ 有，✅ **已接门禁**。构造性：`i11_repetition_decay_cut_positions` / `i11_sequence_override_balls` exit 1；`i11_repetition_decay_balls_ok` 与 sequence 降遍数放行；`i11_ritual_ok` 放行；`i11_ritual_wrong_drill` / `i11_ritual_no_review_from` / `i11_ritual_rounds_3` exit 1 |
...
| I13 | 官方计划排课规则（v37 R4–R6 + v38 W7 内容层 + **v39 W2** + **v44**）：①**建议周下界**——每份计划内 **focused 首次引入** 不得早于 `docs/research/20260818-v39-语义课表.md` §5 该 `(plan, id)` 的建议周（warmup / `reviewFrom` 咬合不计入新引入；缺表或条数 ≠ 现网 `Drills/index.json` 登记数 ⇒ FAIL）。**六轴 scalar 不再卡周**（只作 ② 热身包络）。②热身≤主课——同 session **非咬合**热身 scalar max ≤ focused scalar max（带 `reviewFrom` 的热身豁免，因 R6 上一档末段可难于下一档开课）；③衰减——同 drill 按课次展开球数单调不增（展开计入计划侧 `ballsPerRound` 覆盖），低于完整剂量必须 `decay: true`，计划内首次不得 `decay`；④`reviewFrom` ∈ `Plans/index.json` 且来源计划实际包含该 drill。⑤**引入序（v38 W7）**：focused 首次引入短 id 序 = `docs/research/20260814-v38-先决与主课名单.md` §3 该计划主课表序。**只比序，不比「建议周」**。⑥**周满（v44）**：每个 `week` 的 `sessions.count == plan.sessionsPerWeek`，且 `{dayNumber} == {1..sessionsPerWeek}`。⑦**日合计（v44）**：同 session 计入时长的 `phase.durationMinutes` 之和 ≥ 75；`countsTowardMinutes == false`（或缺省 `type==ritual`）不计入。⑧**主题禁幽灵（v44）**：`week.theme` 与计划根 `description` 不得含子串 `开球`/`九球`/`跳球`/`解球`/`远台中袋`/`握杆`/`站位`/`手架`（大小写不敏感）。 | I13（v37 W5；v38 W7 落地引入序；**v39 W2 改 ①**；**v44 加 ⑥⑦⑧**） | ✅ 有，✅ **已接门禁**（八条）。构造性：`i13_week_order_drop` 把 c032 首次提前到早于建议周 ⇒ exit 1；对调准度Ⅰ W1/W2 首次 focused ⇒ exit 1；`i13_week_missing_day` / `i13_session_under_75` / `i13_ghost_theme` 各 exit 1 |
...
### 8.5 现网 3 个 drill 无任何序列，且本质上录不出

现网登记：`c065`（Ghost Game）、`c068`（五球连打走位）、`c070`（全台清台挑战）。
原因：盘面随机/对抗性（Ghost、清台类）。

2026-08-26 暂时下架后不再出现在 `index.json` 的原豁免：`c008`/`c043`（手架）、`c059`（跳球）、`c061`（解球）、`c067`（九球）。备份见 `content/retired-drills/`。
...
| 版本 | 日期 | 变更 |
|---|---|---|
| 2.19 | 2026-09-14 | **v63 W17-C**：依 ADR-P10-14/15 补 §6.7，明确默认平面裁定、派生袋内呈现、序列杆末/下一杆来源及跨杆导出尚未验收的边界。 |
...
| 2.16 | 2026-08-26 | **v44 W6**：`SessionPhase.countsTowardMinutes` / `PlanDrillDose.ritual`；I11 五分点开场白名单；I13 ③ ritual 首次允许 decay。 |
| 2.15 | 2026-08-26 | **v44 W4**：I13 加 ⑥周满 / ⑦日合计≥75（排除 `countsTowardMinutes==false`）/ ⑧主题禁幽灵词。构造性 `i13_week_missing_day` / `i13_session_under_75` / `i13_ghost_theme`。 |
| 2.14 | 2026-08-26 | 中袋 `c012` / 半台 `c001` 主分类改 `fundamentals`，主课迁入 `plan_beginner`（引入序 c012→c009→c022→c001→c010→c023）。准度Ⅰ主课改为 c011→c013→c032。I13 对照表（v38 §3 / v39 §5）同步。 |
| 2.13 | 2026-08-26 | 暂时下架 10 课（c006/c007/c008/c043/c002/c062/c059/c061/c066/c067）：备份 `content/retired-drills/`，不进 Bundle。I13 主课表/建议周条数改为对齐 `Drills/index.json`（不再钉 84）。§8.5 I9 现网豁免 8→3。c055 留下。 |
