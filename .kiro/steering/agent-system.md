...
**子智能体（`.cursor/agents/*.md`）**：与上表及对应专项规则 **内容对齐**，提供在 Cursor 中 **主动调用** 的入口（独立会话上下文、聚焦系统提示）。**规范性约束**仍以 `.cursor/rules/*.mdc` 与本文档为准；子智能体不另设一套路由表，避免漂移。说明见仓库根目录 [`AGENTS.md`](../../AGENTS.md) 中「子智能体」一节。
