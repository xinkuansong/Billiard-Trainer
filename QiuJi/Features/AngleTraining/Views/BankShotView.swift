...
///
/// W4 版面（20260709 翻袋反射页重构方案 §1.2）：顶部 1 行库数 chip；右缘纯力度柱
/// （`BTShotInstrumentColumn` onSpinTap=nil，力度 = 求解输入）；右下贴边动作列
/// （下一解 / 重置）；底部球库带（拖入 = 障碍球，真实碰撞体进反解模拟）。
struct BankShotView: View {
    ...
    private func paletteDrag(_ key: String) -> some Gesture {
        DragGesture(minimumDistance: 10, coordinateSpace: .named("bankshot"))
            .onChanged { value in
                draggingKey = key
