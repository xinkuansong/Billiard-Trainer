...
enum PositionPlaySolver {
    ...
    static func solve(
        ...
        case let .restRegion(region):
            return solveRestRegion(setup: setup, targetKey: targetKey, pocket: pocket,
                                   region: region, params: params ?? .standard)
        case let .passThrough(point, vMin):
            return solvePassThrough(setup: setup, targetKey: targetKey, pocket: pocket,
                                    point: point, vMin: vMin, params: params ?? .passThrough)
        }
    }
    ...
    private static func solveRestRegion(
        ...
        // 降级：无合格解。优先返回「能进球」的最接近解（signed 最小）；否则返回任意最接近停点。
        if let closest = pottedScored.min(by: { $0.signed < $1.signed }) {
            return [makeRegionSolution(closest.candidate, signed: closest.signed,
                                       cushion: closest.cushion, satisfied: false)]
        }
        ...
        if let f = fallback {
            return [makeRegionSolution(f.candidate, signed: f.signed, cushion: f.cushion, satisfied: false)]
        }
        return []
    }
    ...
    private static func makeRegionSolution(
        _ c: Candidate, signed: Float, cushion: Int, satisfied: Bool
    ) -> PositionPlaySolution {
        let margin = -signed   // 区内深度（正=离边界多远，越大越鲁棒）；区外为负。
        let cm = Int((abs(signed) * 100).rounded())
        let regionText: String
        if c.potted {
            regionText = signed <= 0 ? "停点在落区内 · 余量约 \(cm)cm" : "未落区 · 距落区约 \(cm)cm"
        } else {
            regionText = "未进袋（最接近解）· 距落区约 \(cm)cm"
        }
        let summary = "\(spinText(c.spinX, c.spinY)) · \(PowerDisplay.name(c.velocity)) \(String(format: "%.1f", c.velocity)) · \(cushionText(cushion)) · \(regionText)"
